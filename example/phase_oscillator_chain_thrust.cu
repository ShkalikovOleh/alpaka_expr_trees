...
public:
    struct sys_functor
    {
        ...
        __host__ __device__ void operator()(Tuple t) // this functor works on tuples of values
        {
            // first, unpack the tuple into value, neighbors and omega
            value_type const phi = thrust::get<0>(t);
            value_type const phi_left = thrust::get<1>(t); // left neighbor
            value_type const phi_right = thrust::get<2>(t); // right neighbor
            value_type const omega = thrust::get<3>(t);
            // the dynamical equation
            thrust::get<4>(t) = omega + sin(phi_right - phi) + sin(phi - phi_left);
        }
    };

    phase_oscillators(state_type const& omega)
        : m_omega(omega)
        , m_N(omega.size())
        , m_prev(omega.size())
        , m_next(omega.size())
